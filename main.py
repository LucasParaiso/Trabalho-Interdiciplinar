...
class Application:
    ...
    def nayveBaies(self, event):
        ...
        self.labelEncoderTresUltimasLetras = LabelEncoder()
        ...
        self.baseDados[:,2] = self.labelEncoderTresUltimasLetras.fit_transform(self.baseDados[:,2])
    ...
    def verificaNome(self):
        ...
        for x in range(0, 3):
            ...
            for i in range(0, 1264):
                if (self.nomes[i][-1] == self.verificar[x][-1] and self.nomes[i][-2] == self.verificar[x][-2] and self.nomes[i][-3] == self.verificar[x][-3]):
                    self.nomeDadosTres = self.baseDados[i][1]
                    break

            self.previsao = self.naiveNomes.predict([[self.nomeDadosUm, self.nomeDadosDois, self.nomeDadosTres], [self.nomeDadosUm, self.nomeDadosDois, self.nomeDadosTres]])
